...
public class TestResultSerHandler {
	...
	public void test2() throws SQLException {
		...
		//for（元素类型  元素名称 ： 遍历数组（集合）（或者能进行迭代的））{语句},foreach也叫增强for循环
		for (Object[] os : query) {
			  for (Object o : os) { 
				  System.out.println(o);
			  }
	...
	public void test4() throws SQLException {
		...
		//for (iterable_type iterable_element : iterable) {}迭代器for循环
		
		for (Map.Entry<Object,Map<String,Object>> m : map.entrySet()) {
			System.out.println(m.getKey());//大Map中的key值就是id列的值
	...
	@Test
	//BeanHandler表示把结果集中的一行数据，封装成一个对象，专门针对结果集中只有一行数据的情况。
