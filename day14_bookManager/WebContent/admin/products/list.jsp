...
<script type="text/javascript">
	...
	function checkAll(){
		...
		//alert(flag);//验证一下是否得到了状态值
		//得到所有ids复选框元素，
		var ids=document.getElementsByName("ids");
		//循环给每个复选框赋值
		for (var i=0;i<ids.length;i++){
			ids[i].checked=flag;//把ckAll元素的状态赋值给给一个ids[i]得元素
		}
	}
	//批量删除
...
/* 		//得到所有复选框
		...
		for(var i=0;i<ids.length;i++){
			if(ids[i].checked){
				str+="ids="+ids[i].value+"&";//ids=1001&ids=1002&ids=1003
			}
		}
		str=str.substring(0,str.length-1);
		//alert(str);
		if (str!="")
		window.location.href="${pageContext.request.contextPath}/delAllBooksServlet?"+str;
		 */
		//var form2 =document.getElementById("form2");
		 var form2=document.forms[1];//同上，得到页面中第二个表单
		 //给当前表单中的action属性赋值
		// form2.action="${pageContext.request.contextPath}/delAllBooksServlet";
