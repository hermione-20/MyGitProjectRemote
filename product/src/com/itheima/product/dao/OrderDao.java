...
public class OrderDao {
	...
	public void addOrder(Order order) throws SQLException {
		...
		qr.update(ManagerThreadLocal.getConnection(),"INSERT into orders values(?,?,?,?,?,?,?,?)",
				...
				order.getReceiverPhone(),order.getPaystate(),order.getOrdertime(),
				order.getUser().getId());
	}
//根据用户ID查询所有订单
	...
	//查询指定用户的订单信息
	public static  Order findOrdersByOrderId(String orderid) throws SQLException {
		QueryRunner qr=new QueryRunner(C3P0Util.getDataSource());
       //得到一个订单
		Order order=qr.query("select * from orders where id=?",new BeanHandler<Order>(Order.class),orderid);
	
		//得到当前订单中的所有订单项
      // List<OrderItem> orderItems= qr.query("select * from orderitem where order_id=?", new BeanListHandler<OrderItem>(OrderItem.class));
