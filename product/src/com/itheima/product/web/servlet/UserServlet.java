...
public class UserServlet extends BaseServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		/*
		 * //获取请求参数 String method=request.getParameter("method"); if(!"".equals(method))
		 * { if("login".equals(method)) { login(request, response); }
		 * if("register".equals(method)) { register(request, response); }
		 * if("logout".equals(method)) { logout(request, response); }
		 * if("findUserById".equals(method)) { findUserById(request, response); }
		 * 
		 * 
		 * }
		 */
	...
	public void register(HttpServletRequest request, HttpServletResponse response) 
		...
		//处理验证码
				...
				if (!checkcode_session.equals(ckcode)) {//如果输入的验证码和实际不一致，则返回注册页面
				...
				try {
					...
					//分发转向
					//要求用户激活后才能登陆，因此不能直接把信息保存到session中
